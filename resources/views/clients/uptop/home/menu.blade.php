...
@section('content')
<div id="bg" class="flex flex-col items-center text-white">
    <div class="max-w-6xl py-24 w-full flex flex-col px-6">
        <span class="text-4xl font-bold self-center">MENU</span>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mt-16">
            @foreach ($menu->categories as $category)
            <div class="flex flex-col w-full">
                <span class="harlekin text-5xl mb-7">{{ $category->name }}</span>

                @foreach ($category->items as $item)
                <div @class(['flex justify-between w-full', 'magicshop'=> $item->name == 'Magic Shop!', 'butterbeer' =>
                    $item->name == 'Butter Beer'])>
                    <span>{{ $item->name }}</span>
                ...
                <div class="mt-4 flex flex-col w-full">
                    <span class="font-semibold text-lg">{{ $category->extra->name }}</span>

                    @foreach ($category->extra->addons as $addon)
                    <div class="flex justify-between w-full">
                        <span>{{ $addon->name }}</span>
                        <span>{{ $addon->price ? $addon->price.'.00' : 'Free' }}</span>
                    </div>
                    @endforeach
                </div>
                @endif
...
<style>
    ...
    .magicshop {
        text-shadow: 0px 0px 5px rgba(128, 0, 255, 0.767);
        color: rgb(215, 174, 255)
    }

    .butterbeer {
        text-shadow: 0px 0px 5px rgba(251, 255, 0, 0.836);
