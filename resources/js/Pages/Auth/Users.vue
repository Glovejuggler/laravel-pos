...
<template>
    ...
    <div class="max-w-md mx-auto my-8 dark:text-gray-100">
        <div class="flex justify-end mb-4">
            <button @click="showNewUserModal = true" class="py-1 px-2 text-white bg-green-500 hover:bg-green-700 active:bg-green-800 duration-200 ease-in-out rounded-lg text-xs">New user</button>
        </div>

        <div v-for="user in users" class="dark:bg-gray-800 bg-white p-4 shadow-md group dark:hover:bg-gray-700 duration-200 ease-in-out flex justify-between items-center mb-1">
            <p>{{ user.name }}</p>
            <div class="space-x-2">
                <i @click="editUser(user)" class="bx bx-edit invisible group-hover:visible inline-flex justify-center items-center h-5 w-5 p-4 rounded-full cursor-pointer hover:bg-blue-500"></i>
                <i v-if="user.id !== 1" @click="deleteUser(user)" class="bx bx-trash invisible group-hover:visible inline-flex justify-center items-center h-5 w-5 p-4 rounded-full cursor-pointer hover:bg-red-500"></i>
            </div>
        </div>
    </div>
    ...
    <Modal max-width="sm" :show="showNewUserModal" @close="closeModal">
        <div class="bg-white dark:bg-gray-800 p-4 shadow-lg dark:text-white">
    ...
    <Modal :show="showDeleteUserModal" @close="closeModal" max-width="sm">
        <div class="bg-white dark:bg-gray-800 p-4 dark:text-gray-100">
