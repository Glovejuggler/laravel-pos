...
<template>
    ...
    <div class="max-w-7xl mx-auto py-8 px-6 lg:px-8 dark:text-white">
        ...
        <div v-if="Object.keys(visibleExpense.data).length" class="border dark:border-zinc-700 rounded-lg">
            ...
            <table class="w-full table bg-white dark:bg-zinc-900 dark:text-white rounded-lg overflow-hidden">
                ...
                <tbody>
                        <tr class="dark:odd:bg-zinc-800 odd:bg-zinc-200" v-for="expense in Object.keys(visibleExpense.data)">
                            <td class="px-4 py-2">{{ expense }}</td>
                            <td class="px-4 py-2 space-y-2"><p v-for="item in visibleExpense.data[expense]">{{ item.item }}</p></td>
                            <td class="px-4 py-2 space-y-2"><p v-for="item in visibleExpense.data[expense]">{{ item.amount }}</p></td>
                            <td class="px-4 py-2 space-y-2"><p v-for="item in visibleExpense.data[expense]">{{ item.type }}</p></td>
                            <td class="px-4 py-2 space-y-2">
                                <div v-for="item in visibleExpense.data[expense]" class="flex items-center space-x-2">
                                    <i @click="editExpense(item)" class="bx bx-edit w-6 h-6 inline-flex justify-center items-center bg-green-500 hover:bg-green-600 active:bg-green-800 duration-200 ease-in-out rounded-full"></i>
                                    <i @click="deleteExpense(item)" class="bx bx-trash w-6 h-6 inline-flex justify-center items-center bg-red-500 hover:bg-red-600 active:bg-red-800 duration-200 ease-in-out rounded-full"></i>
                                </div>
                            </td>
                        </tr>
                    </tbody>
